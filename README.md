...
target. It also closes the classic in-page `Runtime.enable` protocol tell as defense-in-depth. See
[`docs/FINDINGS.md`](docs/FINDINGS.md) for what was actually measured, and
[`docs/PRD.md`](docs/PRD.md) for the full architecture.
...
cd chromatrix
pnpm install
pnpm --filter @chromatrix/gateway run start   # boots on :8830, prints the access token once
```
...
- [`docs/PRD.md`](docs/PRD.md) — the product & architecture spec: scoping decisions, the "mitigating mux, not
  transparent proxy" crux, component design. **Start here** for the why.
