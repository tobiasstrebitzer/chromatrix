...
import { profilesRoot, typegenTarget, webDistRoot } from './common/paths.ts'
...
// Dev checkout only: regenerate the committed AppRouter type the web app consumes on every boot. From an npm
// install there is no web *source* to write into (and node_modules may be read-only), so the adapter is
// simply not registered.
const typegenPath = typegenTarget()
  ...
  imports: [
    ...
    SilkweaveModule.forRoot({
      ...
      adapters: [
        ...
        ...(typegenPath ? [typegen({ path: typegenPath })] : []),
      ],
    }),
  ],
